...
abstract class ListSupportAdapter<T> extends RecyclerSupportAdapter<T>
    ...
    @Override
    public void setOnItemDataClickListener(final OnItemDataClickListener<T> onItemClickListener) {

        if (mAbsListView != null && mAbsListView instanceof ListView) {
            mAbsListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    onItemClickListener.onItemClick(view, getItemViewType(position), position, mData);
                }
            });
        } else {
            super.setOnItemDataClickListener(onItemClickListener);
        }
    }
